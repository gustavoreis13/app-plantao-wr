...
const condominios = [ // Lista completa de condomínios com dados atualizados
  ...
  { id: "condo_04", nome: "CDM ABC JF", acessos: [{ descricao: "AnyDesk ID", tipo: "anydesk", valor: "909537479" }], itens_checklist: [{ id_item: "item_04_01", descricao: "CFTV", total_unidades: 254 },{ id_item: "item_04_02", descricao: "CONTROLE DE ACESSO", total_unidades: 2 },{ id_item: "item_04_03", descricao: "CANCELAS", total_unidades: 3 },{ id_item: "item_04_04", descricao: "COMPUTADORES (SRV, MONITORAMENTO E C.A)", total_unidades: 6 }]},
  ...
  { id: "condo_07", nome: "EPAMIG SEDE BH", acessos: [ { descricao: "CIP 850", tipo: "link", valor: "http://201.17.134.15/" }, { descricao: "Situator", tipo: "link", valor: "http://177.69.38.5:10002/app/login" }, { descricao: "D-guard", tipo: "link", valor: "http://177.69.38.5:10000/#!/login" } ], itens_checklist: [{ id_item: "item_07_01", descricao: "CFTV", total_unidades: 24 },{ id_item: "item_07_02", descricao: "CONTROLE DE ACESSO", total_unidades: 6 },{ id_item: "item_07_03", descricao: "COMPUTADORES (SRV, MONITORAMENTO E C.A)", total_unidades: 4 },{ id_item: "item_07_04", descricao: "ATENDEDOR", total_unidades: 3 },{ id_item: "item_07_05", descricao: "SITUATOR", total_unidades: 1 }]},
  { id: "condo_08", nome: "EPAMIG SANTA RITA", acessos: [ { descricao: "NVD 1", tipo: "link", valor: "http://177.129.185.181:10005/" }, { descricao: "XPE 3200 FACE (1)", tipo: "link", valor: "http://177.129.185.181:10007/fcgi/do?id=1" }, { descricao: "XPE 3200 FACE (2)", tipo: "link", valor: "http://177.129.185.181:10008/fcgi/do?id=1" }, { descricao: "TVIP 3000", tipo: "link", valor: "http://177.129.185.181:10012/fcgi/do?id=1" } ], itens_checklist: [{ id_item: "item_08_01", descricao: "CFTV", total_unidades: 16 },{ id_item: "item_08_02", descricao: "CONTROLE DE ACESSO", total_unidades: 2 },{ id_item: "item_08_03", descricao: "CANCELA", total_unidades: 1 }]},
  { id: "condo_09", nome: "EPAMIG UBERABA", acessos: [ { descricao: "NVD 1", tipo: "link", valor: "http://177.69.91.189:10001/" }, { descricao: "XPE 3101", tipo: "link", valor: "http://177.69.91.189:10007/fcgi/do?id=1" } ], itens_checklist: [{ id_item: "item_09_01", descricao: "CFTV", total_unidades: 22 },{ id_item: "item_09_02", descricao: "CONTROLE DE ACESSO", total_unidades: 2 }]},
  { id: "condo_10", nome: "EPAMIG PATROCÍNIO", acessos: [ { descricao: "NVD 1", tipo: "link", valor: "http://177.85.2.154:10003/" }, { descricao: "XPE 3101", tipo: "link", valor: "http://177.85.2.154:7503/fcgi/do?id=1" } ], itens_checklist: [{ id_item: "item_10_01", descricao: "CFTV", total_unidades: 14 },{ id_item: "item_10_02", descricao: "CONTROLE DE ACESSO", total_unidades: 2 }]},
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function atualizarEstadoBotoesEInterface() {
        ...
        const todosOsCondominiosPreenchidos = haRegistros && (registrosDaSessao.length === condominiosComChecklist.length);
    ...
    function gerarRelatorioPDF() { 
        ...
        let pageNumber = 1; const margin = 15; const pageWidth = doc.internal.pageSize.width; const pageHeight = doc.internal.pageSize.height; let currentY = 0; 

        function addPageHeadersAndFooters() { 
            currentY = margin; 
            doc.setFontSize(16); doc.setTextColor(4, 3, 48); doc.text("Relatório de Plantão WR", pageWidth / 2, currentY, { align: "center" }); currentY += 7; 
            doc.setDrawColor(4, 3, 48); doc.line(margin, currentY, pageWidth - margin, currentY); currentY += 7; 
            doc.setFontSize(9); doc.setTextColor(100); doc.text("Página " + pageNumber, pageWidth - margin, pageHeight - 10, {align: 'right'});
        }
        ...
        registrosDaSessao.forEach((registro, idx) => {
            const estimativaAlturaBlocoCondo = 12 + (registro.checklist_resultados.length * 6) + 10; 
            if (currentY + estimativaAlturaBlocoCondo > pageHeight - (margin + 10)) { 
                ...
                doc.text(`Data da Exportação: ${formatDateTime(new Date())}`, margin, currentY); currentY += 9;
            } else if (idx > 0) { currentY += 5; }

            doc.setFontSize(13); doc.setFont('helvetica', 'bold'); doc.text(registro.condominio_nome, margin, currentY); currentY += 6;
            doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.text(`Data da Verificação: ${registro.data_hora_verificacao}`, margin, currentY); currentY += 7; 
            ...
            doc.autoTable({ 
                ...
                columnStyles: { 0: { cellWidth: 55, fontStyle: 'bold' }, 1: { cellWidth: 13, halign: 'center' }, 2: { cellWidth: 18, halign: 'center' }, 3: { cellWidth: 18, halign: 'center' }, 4: { cellWidth: 'auto' }  },
                didDrawPage: function (dataHook) { 
                    pageNumber = dataHook.pageNumber; 
                    addPageHeadersAndFooters(); 
                    currentY = dataHook.cursor.y; 
                }
            });
            currentY = doc.autoTable.previous.finalY + 8; 
        });
        if (doc.internal.getNumberOfPages() === pageNumber) { doc.setPage(pageNumber); addPageHeadersAndFooters(); }
    ...
    // --- AVISO ANTES DE SAIR DA PÁGINA SE HOUVER DADOS NÃO SALVOS ---
    window.addEventListener('beforeunload', function (e) {
        const plantonistaEstaPreenchidoEConfirmado = plantonistaSelect && plantonistaSelect.value !== '' && plantonistaConfirmado;
        // Considera "em progresso" se um condomínio está selecionado para nova entrada E tem itens de checklist renderizados
        const formPossivelmentePreenchido = condominioSelect && condominioSelect.value !== '' && 
                                            editandoRegistroIndex === null && 
                                            checklistContainer && checklistContainer.querySelectorAll('.checklist-item').length > 0;

        if (registrosDaSessao.length > 0 || (plantonistaEstaPreenchidoEConfirmado && formPossivelmentePreenchido)) {
            e.preventDefault(); 
            e.returnValue = 'Você tem alterações não salvas que serão perdidas. Deseja realmente sair?'; 
            return 'Você tem alterações não salvas que serão perdidas. Deseja realmente sair?'; 
        }
    });
